...
class LeadStaffController extends Controller
{
    ...
    public function leadHeadList(Request $request)
    {
        ...
        if($request->export)
        {
            $expData=$query->where('users.account_type','=','lead head')->get();
            return Excel::download(new UsersExport('lead head',$expData), 'LeadHeadList.xlsx');             
    ...
    public function addLeadStaff()
    {
        ...
        $allHead=User::where('account_type','=','lead head')->get();
        $country=Country::get();
        return view('leadStaff.addLeadStaff',compact('country','allHead'));
    }
    ...
    public function leadStaffList(Request $request)
    {
        ...
        if(Auth::user()->account_type=='superadmin')
        {
            if($request->export)
            {
                $expData=$query->where('users.account_type','=','lead staff')->get();
                return Excel::download(new UsersExport('lead staff',$expData), 'LeadStaffList.xlsx');             
        ...
        else
        {
            if($request->export)
            {
                $expData=$query->where([['users.account_type','=','lead staff'],['users.parent_id','=',Auth::user()->id]])->get();
                return Excel::download(new UsersExport('lead staff',$expData), 'LeadStaffList.xlsx');             
    ...
    public function assignBusLead()
    {
        ...
        $allAgent=User::where('account_type','=' ,'lead staff')->get();
    ...
    public function assignBusLeadStore(Request $request)
    {
        ...
        if(isset($request->lead))
        {
            ...
            if($leadStatus==1 || $leadStatus==5)
            {
                ...
                {
                    EnrollCompany::where('id',$lead)->update(['is_assign'=>1,'lead_staff_id'=>$request->agent,'status'=>5]);
                }        
