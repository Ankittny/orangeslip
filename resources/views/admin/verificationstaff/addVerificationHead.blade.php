...
<div class="pxp-dashboard-content-details">
    <h1>Add Verification Head</h1>
    

    <form action="{{ route('storeVerificationHead') }}" method="post" enctype="multipart/form-data">
        {{csrf_field()}}
            ...
            <div class="col-md-6" >
                ...
                <label for="department" class="form-label">Department *</label>
                                <Select   id="department" name="department" required>
                                    ...
                                    @foreach($verification_types as $vt)
                                    <option value="{{$vt->name}}" {{old('department')==$vt->name ? 'selected':''}}>{{$vt->title}}</option>
                                    @endforeach
                                </select>
                    @if ($errors->has('account_type'))
                        <label class="text-danger">{{ $errors->first('account_type') }}</label>
                    @endif
                </div>
            </div>
