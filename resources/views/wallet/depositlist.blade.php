...
@section('content')
    ...
    <div class="pxp-dashboard-content-details">
        ...
        <div class="">
                <div class="">
                    <form name="search" method="get" action="{{url('depositlist')}}">
                    @csrf
                    <div class="row">
                        ...
                        <table id="demo-foo-addrow" class="footable table contact-list">
                            <thead>
                                <tr>
                                    ...
                                    <th data-breakpoints="xs sm">File</th>
                                    @endif
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $w->user?($w->user->business?$w->user->business->business_name:($w->user->first_name.' '.$w->user->last_name)):$w->user_id}}</td>
                                    <td>{{ $w->tid}}</td>
                                    <td>{{ $w->amount}}</td>
                                    <td>{{ $w->comment}}</td>
                                    @if(Auth::user()->account_type!='hr')
                                    <td class="text-warning">
                                       @if($w->doc!='') 
                                    <a href="{{url('images/')}}/{{$w->doc}}" target="_blank">
                                        <img src="{{ ($w->doc!='')?(url('images/'.$w->doc)):(url('images/profile/user.png')) }}" alt="user" width="40" /> 
                                    </a>
                                    @endif
                                    </td>
                                    @endif
                                    ...
                                    <td>
                                        ...
                                        @if((Auth::user()->administrator==1) && ($w->status!=2))
                    ...
                    <h4 class="modal-title">Update Deposit Request Status</h4>
                        ...
                        <form role="form" method="post" id="modelForm">
                            ...
                            <div class="form-group" id="div2" style="display:none">
                                ...
                                <textarea  class="form-control" id="remark" row="3" ></textarea>                           
                            </div>
                            <div class="form-group" id="div3" style="display:none">
                                <label for="inputName">Reason *</label>
                                <textarea  class="form-control" id="reason" row="3" ></textarea>                           
                            </div>
                        </form>
                        <p class="statusMsg"></p>
                </div>
...
//    /$('#modelForm').reset();
 ...
 //console.log(staff_id);
        $.ajax({
            type:'POST',
            url:"{{url('approvedeposit')}}",
            data:'_token='+token+'&deposit_id='+deposit_id+'&response='+response+'&remark='+remark+'&reason='+reason,
            beforeSend: function () {
...
$(document).ready(function(){
		$('.expBtn').click(function() {
            ...
            var file_path = url + '/depositlist?' + formData + '&export=true';  
